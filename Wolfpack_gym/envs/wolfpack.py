        ...
        # Define width and height of grid world
            ...
            if obs_type == "vector":
                ...
                sa_observation_space = spaces.Box(low=np.zeros((self.grid_height, self.grid_width, 3)),
                                                                  high=np.ones((self.grid_height, self.grid_width,3)),
                                                                  dtype=np.int32)
            ...
            for idx, agent in enumerate(self.food_list):
                filename = os.path.join(dirname, ("wolfpack_assets/dqn_prey_parameters/exp0.0001param_10_agent_"+str(idx)))
            ...
            if obs_type == "vector":
                ...
                for i in range(self.grid_height):
                    for j in range(self.grid_width):
                        object_type = self.grid[i][j]
                        if object_type > 1:
                            res[i, j, object_type - 2] = 1
                x,y = self.player_positions[agent_id][0], self.player_positions[agent_id][1]
                res[x,y,2] = 1
                return{'image': res}
            # 2: wolves, 3: sheep --> 0: wolves, 1: sheep, 2: ID
